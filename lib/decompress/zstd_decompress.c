...
ZSTD_decompressSequences_t ZSTD_decompressSequences(int bmi2) {
#if DYNAMIC_BMI2
    if (bmi2) {
...
ZSTD_decompressSequences_t ZSTD_decompressSequencesLong(int bmi2) {
#if DYNAMIC_BMI2
    if (bmi2) {
...
{   /* blockType == blockCompressed */
    ...
    if (frame && dctx->fParams.windowSize > (1<<23))
        return ZSTD_decompressSequencesLong(dctx->bmi2)(dctx, dst, dstCapacity, ip, srcSize, isLongOffset);
    return ZSTD_decompressSequences(dctx->bmi2)(dctx, dst, dstCapacity, ip, srcSize, isLongOffset);
}
