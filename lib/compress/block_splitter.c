...
/**
 * Construct a blockSplit_t.
 * Internally, we store the end sequence, not the number of sequences.
 * It needs to be fixed before returning it.
 */
static blockSplit_t constructBlockSplit(size_t const end,
...
static void fixBlockSplits(blockSplit_t* split, size_t const nbSplits)
{
    size_t nbSeq = 0;
    blockSplit_t const* const splitEnd = split + nbSplits;
...
FORCE_INLINE
size_t ZSTD_blockSplit_internal(blockSplitState_t* const state,
    ...
    size_t const nbWindows = state->nbWindows;
    double const epsilon = computeEpsilon(nbWindows, blockSwitchCost, maxCost);
    window_t *const windows = state->windows;
    pred_t *const pred = state->pred;
    double *const minCost = state->minCost;
    ...
    for (idx = 0, seq = 0; idx < nbSplits; ++idx, seq += stepSize) {
        ...
        for (windowIndex = 0; windowIndex < nbWindows; ++windowIndex) {
            ...
            for (;;) {
                cost_t const cost =
                    computeCost(window, seq, offsets, blockSwitchCost, type);
...
size_t const* initOffsets(size_t* const offsets, seqStore_t const* seqStore,
                          int const type)
{
    ...
    for (seq = 0; seq < nbSeq; ++seq) {
        offsets[seq + 1] = offsets[seq] + seqStore->sequences[seq].litLength;
 ...
 */
static size_t computeMaxCost(seqStore_t const* const seqStore, int const type)
{
    size_t const blockSwitchCost = computeBlockSwitchCost(type);
    size_t const nbSeq = seqStore->sequences - seqStore->sequencesStart;
    size_t const nbLits = seqStore->lit - seqStore->litStart;
...
int compareSplits(void const* lp, void const* rp)
{
    ...
    if (lhs->nbSeq < rhs->nbSeq)
        return -1;
    if (lhs->nbSeq > rhs->nbSeq)
        return 1;
    assert(lhs->nbSeq == rhs->nbSeq);
    return 0;
}

size_t ZSTD_blockSplit(blockSplitState_t* state, seqStore_t const* seqStore)
{
    size_t nbSplits = 0;
    int type;
    ...
    for (type = st_lit; type < st_end; ++type) {
        size_t const* const offsets =
                initOffsets(state->offsets, seqStore, type);
        size_t const blockSwitchCost = computeBlockSwitchCost(type);
        size_t const maxCost = computeMaxCost(seqStore, type);
        // TODO: Check that we don't overrun splits...
        nbSplits += ZSTD_blockSplit_internal(
            state, seqStore, state->splits + nbSplits, state->maxNbSplits,
            blockSwitchCost, maxCost, offsets, type);
    }
    qsort(state->splits, nbSplits, sizeof(blockSplit_t), compareSplits);
    ...
    fixBlockSplits(state->splits, nbSplits);
