...
static void ZSTDMT_serialState_reset(serialState_t* serialState)
{
...
static int ZSTDMT_serialState_init(serialState_t* serialState)
{
  ...
  ZSTDMT_serialState_reset(serialState);
  return initError;
}
...
static void ZSTDMT_serialState_update(serialState_t* serialState, range_t src, unsigned jobID, unsigned checksumFlag)
{
    /* Wait for our turn */
    ...
    assert(serialState->nextJobID == jobID);
    /* It is now our turn, do any processing necessary */
    if (checksumFlag) XXH64_update(&serialState->xxhState, src.start, src.size);
...
void ZSTDMT_compressionJob(void* jobDescription)
{
    ...
    ZSTDMT_serialState_update(job->serial, job->src, job->jobID,
                              jobParams.fParams.checksumFlag);
...
ZSTDMT_CCtx* ZSTDMT_createCCtx_advanced(unsigned nbWorkers, ZSTD_customMem cMem)
{
    ...
    mtctx->inBuffSpace = g_nullBuffer;
...
size_t ZSTDMT_freeCCtx(ZSTDMT_CCtx* mtctx)
{
    ...
    if (mtctx->inBuffSpace.start)
        ZSTD_free(mtctx->inBuffSpace.start, mtctx->cMem);
...
static size_t ZSTDMT_computeTargetJobSize(ZSTD_CCtx_params const params)
{
...
static size_t ZSTDMT_computeOverlapSize(ZSTD_CCtx_params const params)
{
    ...
    return overlapRLog >= 9 ? 0 : (size_t)1 << (params.cParams.windowLog - overlapRLog);
}

static unsigned ZSTDMT_computeNbJobs(ZSTD_CCtx_params params, size_t srcSize, unsigned nbWorkers) {
    assert(nbWorkers>0);
    {   size_t const jobSizeTarget = ZSTDMT_computeTargetJobSize(params);
                ...
                ZSTD_CCtx_params const params)
{
    ...
    ZSTDMT_serialState_reset(&mtctx->serial);
        ...
        unsigned long long pledgedSrcSize)
{
    ...
    if (params.jobSize == 0) {
        if (params.cParams.windowLog >= 29)
            ...
            params.jobSize = ZSTDMT_computeTargetJobSize(params);
    }
    ...
    ZSTDMT_setBufferSize(mtctx->bufPool, ZSTD_compressBound(mtctx->targetSectionSize));
    {
        ...
        if (mtctx->inBuffSpace.capacity < capacity) {
            if (mtctx->inBuffSpace.start)
                ZSTD_free(mtctx->inBuffSpace.start, mtctx->cMem);
            mtctx->inBuffSpace.start = ZSTD_malloc(capacity, mtctx->cMem);
            if (mtctx->inBuffSpace.start == NULL) {
                mtctx->inBuffSpace.capacity = 0;
                return ERROR(memory_allocation);
            }
            mtctx->inBuffSpace.capacity = capacity;
        }
    }
    DEBUGLOG(4, "inBuffSpace : %u KB", (U32)(mtctx->inBuffSpace.capacity>>10));
    mtctx->inBuffPos = 0;
    mtctx->inBuff.buffer = g_nullBuffer;
 ...
 * If the earliest job has two sections of
 * If the data of the first  is broken up into two segments, we cover both sections.
 */
static range_t ZSTDMT_getInputDataInUse(ZSTDMT_CCtx* mtctx)
{
    ...
    for (jobID = firstJobID; jobID < lastJobID; ++jobID) {
        ...
        if (consumed < mtctx->jobs[wJobID].src.size) {
            ...
            } else if (mtctx->jobs[wJobID].src.start < range.start) {
                /* Two segments */
                assert(mtctx->jobs[wJobID].src.start == mtctx->inBuffSpace.start);
                range.start = mtctx->inBuffSpace.start;
                range.size = mtctx->inBuffSpace.capacity;
            }
            return range;
        }
    }
...
static int ZSTDMT_isOverlapped(buffer_t buffer, range_t range)
{
    ...
    if (bufferStart <= rangeStart && bufferEnd > rangeStart)
        return 1;
    if (bufferStart < rangeEnd && bufferEnd >= rangeEnd)
        return 1;
    if (bufferStart > rangeStart && bufferEnd <= rangeEnd)
        return 1;
...
static int ZSTDMT_tryGetInputRange(ZSTDMT_CCtx* mtctx)
{
    ...
    size_t const spaceLeft = mtctx->inBuffSpace.capacity - mtctx->inBuffPos;
    size_t const target = mtctx->targetSectionSize;
    buffer_t buffer;

    assert(mtctx->inBuff.buffer.start == NULL);
    assert(mtctx->inBuffSpace.capacity >= target);

    if (spaceLeft < target) {
        /* ZSTD_invalidateRepCodes() doesn't work for extDict variants.
         * Simply copy the prefix to the beginning in that case.
         */
        BYTE* const start = (BYTE*)mtctx->inBuffSpace.start;
        size_t const prefixSize = mtctx->inBuff.prefix.size;
        if (inUse.size > 0 && (BYTE const*)inUse.start < start + prefixSize) {
            DEBUGLOG(6, "Waiting for buffer...");
            return 0;
        }
        memmove(start, mtctx->inBuff.prefix.start, prefixSize);
        mtctx->inBuff.prefix.start = start;
        mtctx->inBuffPos = prefixSize;
    }
    buffer.start = (BYTE*)mtctx->inBuffSpace.start + mtctx->inBuffPos;
    ...
    mtctx->inBuff.filled = 0;
    assert(mtctx->inBuffPos + buffer.capacity <= mtctx->inBuffSpace.capacity);
    return 1;
}
                                     ...
                                     ZSTD_EndDirective endOp)
{
    ...
    if ( (mtctx->jobReady)
      ...
      || ((endOp == ZSTD_e_end) && (!mtctx->frameEnded)) ) {   /* must finish the frame with a zero-size block */
        size_t const jobSize = MIN(mtctx->inBuff.filled, mtctx->targetSectionSize);
