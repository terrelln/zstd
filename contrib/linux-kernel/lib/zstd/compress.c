...
ZSTD_compressionParameters ZSTD_adjustCParams(ZSTD_compressionParameters cPar, unsigned long long srcSize,
					      size_t dictSize)
{
...
static size_t ZSTD_resetCCtx_advanced(ZSTD_CCtx *zc, ZSTD_parameters params, U64 frameContentSize,
				      ZSTD_compResetPolicy_e const crp)
{
...

	{
		...
		U32 const hashLog3 =
		    (params.cParams.searchLength > 3) ? 0 : MIN(ZSTD_HASHLOG3_MAX, params.cParams.windowLog);
		size_t const h3Size = ((size_t)1) << hashLog3;
		size_t const tableSpace = (chainSize + hSize + h3Size) * sizeof(U32);
		void *ptr;
...
#define LITERAL_NOENTROPY 63
		...
		return ERROR(dstSize_tooSmall); /* not enough space for compression */
	{
		...
		if (repeat == HUF_repeat_valid && lhSize == 3)
			singleStream = 1;
		cLitSize = singleStream ? HUF_compress1X_repeat(ostart + lhSize, dstCapacity - lhSize, src, srcSize,
...
MEM_STATIC size_t ZSTD_compressSequences(ZSTD_CCtx *zc, void *dst, size_t dstCapacity, size_t srcSize)
{
	...
	/* CTable for Offsets */
	{
		...
		if ((mostFrequent == nbSeq) && (nbSeq > 2)) {
			*op++ = ofCodeTable[0];
			...
			FSE_buildCTable_wksp(CTable_OffsetBits, norm, max, tableLog, scratchBuffer,
					     sizeof(scratchBuffer));
	...
	/* CTable for MatchLengths */
	{
		...
		if ((mostFrequent == nbSeq) && (nbSeq > 2)) {
			*op++ = *mlCodeTable;
			...
			FSE_buildCTable_wksp(CTable_MatchLength, ML_defaultNorm, MaxML, ML_defaultNormLog,
					     scratchBuffer, sizeof(scratchBuffer));
			MLtype = set_basic;
		} else {
			size_t nbSeq_1 = nbSeq;
			...
			FSE_buildCTable_wksp(CTable_MatchLength, norm, max, tableLog, scratchBuffer,
					     sizeof(scratchBuffer));
...
*/
MEM_STATIC void ZSTD_storeSeq(seqStore_t *seqStorePtr, size_t litLength, const void *literals, U32 offsetCode,
			      size_t matchCode)
{
	...
	if (g_startDebug) {
		...
		if ((pos > 1895000) && (pos < 1895300))
			fprintf(stderr, "Cpos %6u :%5u literals & match %3u bytes at distance %6u \n", pos,
				(U32)litLength, (U32)matchCode + MINMATCH, (U32)offsetCode);
	}
#endif
	/* copy Literals */
...
*/
static size_t ZSTD_count_2segments(const BYTE *ip, const BYTE *match, const BYTE *iEnd, const BYTE *mEnd,
				   const BYTE *iStart)
{
...
static void ZSTD_compressBlock_fast_extDict_generic(ZSTD_CCtx *ctx, const void *src, size_t srcSize, const U32 mls)
{
	...
	while (ip < ilimit) { /* < instead of <=, because (ip+1) */
		...
		if ((((U32)((dictLimit - 1) - repIndex) >= 3) /* intentional underflow */ & (repIndex > lowestIndex)) &&
			...
			mLength = ZSTD_count_2segments(ip + 1 + EQUAL_READ32, repMatch + EQUAL_READ32, iend,
			...
			}
			{
				...
				U32 offset;
				mLength = ZSTD_count_2segments(ip + EQUAL_READ32, match + EQUAL_READ32, iend, matchEnd,
							       lowPrefixPtr) +
					  EQUAL_READ32;
		...
		if (ip <= ilimit) {
			...
			while (ip <= ilimit) {
				...
				if ((((U32)((dictLimit - 1) - repIndex2) >= 3) &
					...
					size_t repLength2 =
					    ZSTD_count_2segments(ip + EQUAL_READ32, repMatch2 + EQUAL_READ32, iend,
								 repEnd2, lowPrefixPtr) +
					    EQUAL_READ32;
...
void ZSTD_compressBlock_doubleFast_generic(ZSTD_CCtx *cctx, const void *src, size_t srcSize, const U32 mls)
{
	...
	while (ip < ilimit) { /* < instead of <=, because repcode check at (ip+1) */
		...
		if ((offset_1 > 0) & (MEM_read32(ip + 1 - offset_1) ==
				      MEM_read32(ip + 1))) { /* note : by construction, offset_1 <= curr */
			mLength = ZSTD_count(ip + 1 + 4, ip + 1 + 4 - offset_1, iend) + 4;
			ip++;
			ZSTD_storeSeq(seqStorePtr, ip - anchor, anchor, 0, mLength - MINMATCH);
		} else {
		...
		if (ip <= ilimit) {
			...
			hashLong[ZSTD_hashPtr(base + curr + 2, hBitsL, 8)] =
				...
				curr + 2; /* here because curr+2 could be > iend-8 */
			hashLong[ZSTD_hashPtr(ip - 2, hBitsL, 8)] = hashSmall[ZSTD_hashPtr(ip - 2, hBitsS, mls)] =
			    (U32)(ip - 2 - base);
...
static void ZSTD_compressBlock_doubleFast_extDict_generic(ZSTD_CCtx *ctx, const void *src, size_t srcSize,
							  const U32 mls)
{
		...
		if ((((U32)((dictLimit - 1) - repIndex) >= 3) /* intentional underflow */ & (repIndex > lowestIndex)) &&
			...
			if ((matchLongIndex > lowestIndex) && (MEM_read64(matchLong) == MEM_read64(ip))) {
				...
				if ((matchIndex3 > lowestIndex) && (MEM_read64(match3) == MEM_read64(ip + 1))) {
					...
					mLength =
					    ZSTD_count_2segments(ip + 9, match3 + 8, iend, matchEnd, lowPrefixPtr) + 8;
					ip++;
					...
					const BYTE *lowMatchPtr = matchIndex < dictLimit ? dictStart : lowPrefixPtr;
					mLength =
					    ZSTD_count_2segments(ip + 4, match + 4, iend, matchEnd, lowPrefixPtr) + 4;
					offset = curr - matchIndex;
					while (((ip > anchor) & (match > lowMatchPtr)) && (ip[-1] == match[-1])) {
						ip--;
						match--;
...
static void ZSTD_updateTree(ZSTD_CCtx *zc, const BYTE *const ip, const BYTE *const iend, const U32 nbCompares,
			    const U32 mls)
{
	const BYTE *const base = zc->base;
	...
	for (; (matchIndex > lowLimit) & (nbAttempts > 0); nbAttempts--) {
		...
		if ((!extDict) || matchIndex >= dictLimit) {
			...
			if (MEM_read32(match) ==
				...
				currMl = ZSTD_count_2segments(ip + EQUAL_READ32, match + EQUAL_READ32, iLimit, dictEnd,
							      prefixStart) +
					 EQUAL_READ32;
		}
				     ...
				     const U32 depth)
{
	...
	while (ip < ilimit) {
		...
		if ((offset_1 > 0) & (MEM_read32(ip + 1) == MEM_read32(ip + 1 - offset_1))) {
			/* repcode : we take it */
			matchLength =
			    ZSTD_count(ip + 1 + EQUAL_READ32, ip + 1 + EQUAL_READ32 - offset_1, iend) + EQUAL_READ32;
			if (depth == 0)
				goto _storeSequence;
		}
		...
		if (depth >= 1)
			while (ip < ilimit) {
				...
				}
				{
					...
					int const gain2 =
					    (int)(ml2 * 4 - ZSTD_highbit32((U32)offset2 + 1)); /* raw approx */
		...
		if (offset) {
			while ((start > anchor) && (start > base + offset - ZSTD_REP_MOVE) &&
			       (start[-1] ==
				start[-1 - offset + ZSTD_REP_MOVE])) /* only search for offset within prefix */
			{
				start--;
				matchLength++;
			}
		...
		/* check repCode */
		{
			...
			if (((U32)((dictLimit - 1) - repIndex) >= 3) &
				...
				if (MEM_read32(ip + 1) == MEM_read32(repMatch)) {
					...
					matchLength =
					    ZSTD_count_2segments(ip + 1 + EQUAL_READ32, repMatch + EQUAL_READ32, iend,
								 repEnd, prefixStart) +
					    EQUAL_READ32;
					if (depth == 0)
						goto _storeSequence;
		...
		while (ip <= ilimit) {
			...
			if (((U32)((dictLimit - 1) - repIndex) >= 3) &
				...
				if (MEM_read32(ip) == MEM_read32(repMatch)) {
					/* repcode detected we should take it */
					const BYTE *const repEnd = repIndex < dictLimit ? dictEnd : iend;
					matchLength = ZSTD_count_2segments(ip + EQUAL_READ32, repMatch + EQUAL_READ32,
...
static ZSTD_blockCompressor ZSTD_selectBlockCompressor(ZSTD_strategy strat, int extDict)
{
	static const ZSTD_blockCompressor blockCompressor[2][8] = {
	    {ZSTD_compressBlock_fast, ZSTD_compressBlock_doubleFast, ZSTD_compressBlock_greedy, ZSTD_compressBlock_lazy,
	     ZSTD_compressBlock_lazy2, ZSTD_compressBlock_btlazy2, ZSTD_compressBlock_btopt, ZSTD_compressBlock_btopt2},
				    ...
				    U32 lastFrameChunk)
{
	...
	while (remaining) {
		...
		if (cctx->lowLimit > (3U << 29)) {
			U32 const cycleMask =
			    (1 << ZSTD_cycleLog(cctx->params.cParams.hashLog, cctx->params.cParams.strategy)) - 1;
...
static size_t ZSTD_writeFrameHeader(void *dst, size_t dstCapacity, ZSTD_parameters params, U64 pledgedSrcSize,
				    U32 dictID)
{
...
static size_t ZSTD_compressContinue_internal(ZSTD_CCtx *cctx, void *dst, size_t dstCapacity, const void *src,
					     size_t srcSize, U32 frame, U32 lastFrameChunk)
{
	...
	if ((ip + srcSize > cctx->dictBase + cctx->lowLimit) & (ip < cctx->dictBase + cctx->dictLimit)) {
		...
		U32 const lowLimitMax =
		    (highInputIdx > (ptrdiff_t)cctx->dictLimit) ? cctx->dictLimit : (U32)highInputIdx;
...
static size_t ZSTD_loadZstdDictionary(ZSTD_CCtx *cctx, const void *dict, size_t dictSize)
{
...

	{
		...
		CHECK_E(FSE_buildCTable_wksp(cctx->offcodeCTable, offcodeNCount, offcodeMaxValue, offcodeLog,
					     scratchBuffer, sizeof(scratchBuffer)),
...

	{
		...
		CHECK_E(FSE_buildCTable_wksp(cctx->matchlengthCTable, matchlengthNCount, matchlengthMaxValue,
					     matchlengthLog, scratchBuffer, sizeof(scratchBuffer)),
			dictionary_corrupted);
...

	{
		...
		CHECK_E(FSE_buildCTable_wksp(cctx->litlengthCTable, litlengthNCount, litlengthMaxValue, litlengthLog,
					     scratchBuffer, sizeof(scratchBuffer)),
			dictionary_corrupted);
...

	{
...

		{
			size_t const errorCode =
			    ZSTD_compressBegin_advanced(cctx, cdict->dictContent, dictSize, params, 0);
...
ZSTD_CDict *ZSTD_initCDict(const void *dict, size_t dictSize, ZSTD_parameters params, void *workspace,
			   size_t workspaceSize)
{
	ZSTD_customMem const stackMem = ZSTD_initStack(workspace, workspaceSize);
...
ZSTD_CStream *ZSTD_initCStream(ZSTD_parameters params, unsigned long long pledgedSrcSize, void *workspace,
			       size_t workspaceSize)
{
	ZSTD_customMem const stackMem = ZSTD_initStack(workspace, workspaceSize);
...
static size_t ZSTD_compressStream_generic(ZSTD_CStream *zcs, void *dst, size_t *dstCapacityPtr, const void *src,
					  size_t *srcSizePtr, ZSTD_flush_e const flush)
{
	U32 someMoreWork = 1;
	...
	while (someMoreWork) {
		...
		case zcss_load:
			...
			/* compress current block (note : this stage cannot be stopped in the middle) */
			{
				...
				if (zcs->inBuffTarget > zcs->inBuffSize)
					zcs->inBuffPos = 0,
					zcs->inBuffTarget = zcs->blockSize; /* note : inBuffSize >= blockSize */
		...
		case zcss_flush: {
			...
			size_t const flushed =
			    ZSTD_limitCopy(op, oend - op, zcs->outBuff + zcs->outBuffFlushedSize, toFlush);
			op += flushed;
...
size_t ZSTD_flushStream(ZSTD_CStream *zcs, ZSTD_outBuffer *output)
{
	...
	size_t sizeWritten = output->size - output->pos;
	size_t const result =
	    ZSTD_compressStream_generic(zcs, (char *)(output->dst) + output->pos, &sizeWritten, &srcSize,
...
size_t ZSTD_endStream(ZSTD_CStream *zcs, ZSTD_outBuffer *output)
{
	...
	if (zcs->stage != zcss_final) {
		...
		zcs->outBuffContentSize =
		    !notEnded ? 0 : ZSTD_compressEnd(zcs->cctx, zcs->outBuff, zcs->outBuffSize, NULL,
