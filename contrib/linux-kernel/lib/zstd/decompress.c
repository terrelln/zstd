...
struct ZSTD_DCtx_s {
	...
	blockType_e
	    bType; /* used in ZSTD_decompressContinue(), to transfer blockType between header decoding and block
		      decoding stages */
	ZSTD_dStage stage;
	U32 litEntropy;
	U32 fseEntropy;
...
static size_t ZSTD_frameHeaderSize(const void *src, size_t srcSize)
{
		...
		return ERROR(srcSize_wrong);
	{
		...
		return ZSTD_frameHeaderSize_prefix + !singleSegment + ZSTD_did_fieldSize[dictID] +
...
{
	{
		...
		while (srcSize >= ZSTD_frameHeaderSize_prefix) {
			...
			if ((magicNumber & 0xFFFFFFF0U) == ZSTD_MAGIC_SKIPPABLE_START) {
				...
				if (srcSize < ZSTD_skippableHeaderSize)
					return ERROR(srcSize_wrong);
				skippableSize = MEM_readLE32((const BYTE *)src + 4) + ZSTD_skippableHeaderSize;
...
size_t ZSTD_decodeLiteralsBlock(ZSTD_DCtx *dctx, const void *src, size_t srcSize) /* note : srcSize < BLOCKSIZE */
{
...

	{
		...
		case set_compressed:
			if (srcSize < 5)
				return ERROR(corruption_detected); /* srcSize >= MIN_CBLOCK_SIZE == 3; here we need up
								      to 5 for case 3 */
			{
				...
				if (HUF_isError(
					(litEncType == set_repeat)
					    ? (singleStream ? HUF_decompress1X_usingDTable(dctx->litBuffer, litSize,
											   istart + lhSize, litCSize,
											   dctx->HUFptr)
							    : HUF_decompress4X_usingDTable(dctx->litBuffer, litSize,
											   istart + lhSize, litCSize,
											   dctx->HUFptr))
					    : (singleStream
						   ? HUF_decompress1X2_DCtx(dctx->entropy.hufTable, dctx->litBuffer,
									    litSize, istart + lhSize, litCSize)
						   : HUF_decompress4X_hufOnly(dctx->entropy.hufTable, dctx->litBuffer,
									      litSize, istart + lhSize, litCSize))))
					return ERROR(corruption_detected);
		...
		case set_rle: {
			...
			case 3:
				...
				litSize = MEM_readLE24(istart) >> 4;
				if (srcSize < 4)
					return ERROR(corruption_detected); /* srcSize >= MIN_CBLOCK_SIZE == 3; here we
									      need lhSize+1 = 4 */
				break;
			}
			if (litSize > ZSTD_BLOCKSIZE_ABSOLUTEMAX)
...
static size_t ZSTD_buildSeqTable(FSE_DTable *DTableSpace, const FSE_DTable **DTablePtr, symbolEncodingType_e type,
				 U32 max, U32 maxLog, const void *src, size_t srcSize,
		...
		return ERROR(srcSize_wrong); /* minimum possible size */
	{
		...
		}
		{
			size_t const ofhSize =
			    ZSTD_buildSeqTable(dctx->entropy.OFTable, &dctx->OFTptr, OFtype, MaxOff, OffFSELog, ip,
...
size_t ZSTD_execSequenceLast7(BYTE *op, BYTE *const oend, seq_t sequence, const BYTE **litPtr,
			      const BYTE *const litLimit, const BYTE *const base, const BYTE *const vBase,
			      const BYTE *const dictEnd)
{
	...
	/* check */
	if (oMatchEnd > oend)
		return ERROR(dstSize_tooSmall); /* last match must start at a minimum distance of WILDCOPY_OVERLENGTH
						   from oend */
	if (iLitEnd > litLimit)
	...
	/* sequence */
	{
		...
		if (!ofCode)
			...
			offset = OF_base[ofCode] +
				 BIT_readBitsFast(&seqState->DStream, ofBits); /* <=  (ZSTD_WINDOWLOG_MAX-1) bits */
			if (MEM_32bits())
...
#define ADVANCED_SEQS 4
		...
		for (; (BIT_reloadDStream(&(seqState.DStream)) <= BIT_DStream_completed) && seqNb < nbSeq; seqNb++) {
			...
			size_t const oneSeqSize =
			    ZSTD_execSequenceLong(op, oend, sequences[(seqNb - ADVANCED_SEQS) & STOSEQ_MASK], &litPtr,
		...
		for (; seqNb < nbSeq; seqNb++) {
			size_t const oneSeqSize = ZSTD_execSequenceLong(op, oend, sequences[seqNb & STOSEQ_MASK],
...
{
	if (srcSize >= ZSTD_skippableHeaderSize && (MEM_readLE32(src) & 0xFFFFFFF0U) == ZSTD_MAGIC_SKIPPABLE_START) {
...
static size_t ZSTD_decompressFrame(ZSTD_DCtx *dctx, void *dst, size_t dstCapacity, const void **srcPtr,
				   size_t *srcSizePtr)
{
...
static size_t ZSTD_loadEntropy(ZSTD_entropyTables_t *entropy, const void *const dict, size_t const dictSize)
{
...

	{
		...
		if (offcodeLog > OffFSELog)
			return ERROR(dictionary_corrupted);
		CHECK_E(FSE_buildDTable(entropy->OFTable, offcodeNCount, offcodeMaxValue, offcodeLog),
...

	{
		...
		if (matchlengthLog > MLFSELog)
			return ERROR(dictionary_corrupted);
		CHECK_E(FSE_buildDTable(entropy->MLTable, matchlengthNCount, matchlengthMaxValue, matchlengthLog),
			dictionary_corrupted);
...

	{
		...
		if (litlengthLog > LLFSELog)
			return ERROR(dictionary_corrupted);
		CHECK_E(FSE_buildDTable(entropy->LLTable, litlengthNCount, litlengthMaxValue, litlengthLog),
			dictionary_corrupted);
...
static ZSTD_DDict *ZSTD_createDDict_advanced(const void *dict, size_t dictSize, unsigned byReference,
					     ZSTD_customMem customMem)
{
	if (!customMem.customAlloc || !customMem.customFree)
		return NULL;
 ...
 *  When identifying the exact failure cause, it's possible to used ZSTD_getFrameParams(), which will provide a more
 * precise error code. */
...
ZSTD_DStream *ZSTD_initDStream_usingDDict(size_t maxWindowSize, const ZSTD_DDict *ddict, void *workspace,
...
size_t ZSTD_decompressStream(ZSTD_DStream *zds, ZSTD_outBuffer *output, ZSTD_inBuffer *input)
{
	...
	while (someMoreWork) {
		...
		case zdss_loadHeader: {
			...
			/* Adapt buffer sizes to frame header instructions */
			{
				...
				size_t const neededOutSize =
				    zds->fParams.windowSize + blockSize + WILDCOPY_OVERLENGTH * 2;
		...
		case zdss_load: {
			size_t const neededInSize = ZSTD_nextSrcSizeToDecompress(zds->dctx);
			size_t const toLoad =
			    neededInSize - zds->inPos; /* should always be <= remaining space within inBuff */
			size_t loadedSize;
			...
			/* decode loaded input */
			{
				const int isSkipFrame = ZSTD_isSkipFrame(zds->dctx);
				size_t const decodedSize = ZSTD_decompressContinue(
				    zds->dctx, zds->outBuff + zds->outStart, zds->outBuffSize - zds->outStart,
	...
	output->pos += (size_t)(op - ostart);
	{
		...
		nextSrcSizeHint += ZSTD_blockHeaderSize *
				   (ZSTD_nextInputType(zds->dctx) == ZSTDnit_block); /* preload header of next block */
