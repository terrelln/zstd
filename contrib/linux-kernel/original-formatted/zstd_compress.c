...
ZSTD_CCtx *ZSTD_createCCtx(void) { return ZSTD_createCCtx_advanced(defaultCustomMem); }
...
static size_t ZSTD_compress_generic(ZSTD_CCtx *cctx, void *dst, size_t dstCapacity, const void *src, size_t srcSize, U32 lastFrameChunk)
{
	...
	while (remaining) {
		...
		if (cctx->lowLimit > (3U << 29)) {
			...
			U32 const newCurrent = (curr & cycleMask) + (1 << cctx->params.cParams.windowLog);
...
size_t ZSTD_compress_advanced(ZSTD_CCtx *ctx, void *dst, size_t dstCapacity, const void *src, size_t srcSize, const void *dict, size_t dictSize,
			      ZSTD_parameters params)
{
	CHECK_F(ZSTD_checkCParams(params.cParams));
...
ZSTD_CStream *ZSTD_createCStream(void) { return ZSTD_createCStream_advanced(defaultCustomMem); }
...
size_t ZSTD_initCStream(ZSTD_CStream *zcs, int compressionLevel) { return ZSTD_initCStream_usingDict(zcs, NULL, 0, compressionLevel); }
