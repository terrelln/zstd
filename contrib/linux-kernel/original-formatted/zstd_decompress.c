...
ZSTD_DCtx *ZSTD_createDCtx(void) { return ZSTD_createDCtx_advanced(defaultCustomMem); }
...
ZSTD_DStream *ZSTD_createDStream(void) { return ZSTD_createDStream_advanced(defaultCustomMem); }
...
size_t ZSTD_initDStream(ZSTD_DStream *zds) { return ZSTD_initDStream_usingDict(zds, NULL, 0); }
		...
		case zdss_loadHeader: {
...
#if defined(ZSTD_LEGACY_SUPPORT) && (ZSTD_LEGACY_SUPPORT >= 1)
			{
				...
				if (legacyVersion) {
					...
					CHECK_F(ZSTD_initLegacyStream(&zds->legacyContext, zds->previousLegacyVersion, legacyVersion, dict, dictSize));
